...
void runSwiftUCRLgamma(MDP_type &mdp, int S, int _nA)
{
	...
	int t = 0;
	//T=10000;
	int reps = 1; // plotting replicates
	...
	for (int rep = 0; rep < reps; rep++)
	{
		...
		std::cout << "Swift_UCRL_Gamma policy:  ";
		for (int i: _policy) {
			std::cout << i;
		}	
		 std::cout << std::endl;
	...
	if (make_plots) {
		...
		filename << "pyplotfiles/swift_ucrlg_v_opt_" << nS << "_" << nA <<".txt";
		ofstream topython;
		...
		filename1 << "pyplotfiles/swift_ucrlg_v_pol_" << nS << "_" << nA <<".txt";
		...
		filename2 << "pyplotfiles/swift_ucrlg_v_pol_e" << nS << "_" << nA <<".txt";
		...
		filename3 << "pyplotfiles/swift_ucrlg_v_opt_e" << nS << "_" << nA <<".txt";
...
void RLRS(string filename, int expnum, int States, int Actions, int SS, int StartP, int endP, int IncP, double epsilon, double gamma, double upper_reward, double non_zero_transition)
{
	...
	for (int iters = 0; iters < repetitions; iters++)
	{
		...
		for (int ite = StartP; ite <= endP; ite = ite + IncP)
		{
			//threads.push_back(std::thread([&,ite, k]() mutable { 
				...
				A_type A5 = copy_A(A);
				auto start_swift_UCRL_G = high_resolution_clock::now();
				runSwiftUCRLgamma(MDP, S, nA);
				auto stop_swift_UCRL_G = high_resolution_clock::now();
				auto duration_swift_UCRL_G= duration_cast<milliseconds>(stop_swift_UCRL_G - start_swift_UCRL_G);
				...
				VI[4][k] += duration_swift_UCRL_G.count();
				//std::cout << k << std::endl;
				//std::cout << VI[0][k] << std::endl;
				string_stream << duration_VI.count() << " " << duration_VIH.count()<< " " << duration_BAO.count() << " " << duration_UCRL_G.count() << " " << duration_swift_UCRL_G.count() <<endl;
			//}));
			k++;
			//std::cout << VI[0][k] << std::endl;
		}
	...
	{
		avgstring_stream << VI[0][k] / repetitions << " " << VI[1][k] / repetitions << " " << VI[2][k] / repetitions << " " << VI[3][k] / repetitions << " " << VI[4][k] / repetitions  <<  endl;
	}
